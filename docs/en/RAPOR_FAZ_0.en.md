<!-- Machine translation of `numcog/RAPOR_FAZ_0.md` (local Ollama / gemma3:4b). The Turkish original is the authoritative record; this file is a reading copy.
     Numeric-token check: PASS (all 174 numeric tokens present) -->

# RAPOR FAZ 0 — Veri keşfi: VPN (görsel projeksiyon nöronu) → KC bağlantısı

Tarih: 2026-09-19. Durum: tamamlandı. Hipotezler ön-kayıtlı (commit `364285d`), ölçümler onlardan sonra yapıldı.

## 1. Yöntem

Ölçüm, `flysim.py` / `cognitive_matrix.py` / `sniff.py`'yi **import etmeden** veri dosyalarını doğrudan pandas ile okur (o üç modülün değişikliğinden bağımsız kalsın diye):

- `proofread_connections_783.feather` → 16.847.997 kenar; sütunlar `pre_pt_root_id, post_pt_root_id, neuropil, syn_count, …`.
- `annotations_783.tsv` → 139.248 satır; `super_class, cell_class, cell_sub_class, cell_type, hemibrain_type, side, top_nt`.

Kümeler: `cell_class == "Kenyon_Cell"` (KC), `cell_class == "ALPN"`, `super_class == "visual_projection"` (VPN). VPN→KC = `pre ∈ VPN ∧ post ∈ KC` kenarları.
Betikler: `numcog/explore_p0.py` (taksonomi + bileşim), `explore_p0b.py` (VPN + MBIN), `explore_p0c.py` (kesin ölçüm), `explore_p0d.py` (KCg-d ayrıştırması). Ham kenarlar `numcog/results_p0/*.csv`.
...
VPN→KC ≈ ALPN→KC'nin %7'si (kenar), %4.2'si (sinaps). Gerçek ve ölçülebilir ama küçük bir görsel kanal var.
...
Karşılaştırma — ALPN→KC geniş dağılır: αβ 1.709 KC, γ 2.263 KC, α'β' 915 KC. VPN→KC ise **neredyeyse yalnızca γ-d** (ve ikincil αβ-p) KC'lerine iner. H0.2 doğrulandı.
...
- This is a **simulation/measurement**; not a live bee (see `SINIRLILIKLAR.md`, Phase 6).
- VPN→KC connections’ compartmental level (dendrite or axon) is not available — only
  neuropil is present. PLP/SCL location is compatible with γ-d dendrite region but is not a
  proven dendritic input.
- 7.949/8.038 VPN `cell_class` level is empty (only marked with `super_class`); therefore
  VPN detection was performed with `super_class == "visual_projection"`.
- `syn_count` is the sole proxy for synaptic connection strength; synaptic sign (stimulatory/inhibitory)
...
```bash
cd flyputer
.venv/Scripts/python.exe -X utf8 numcog/explore_p0.py      # taxonomy + assembly
.venv/Scripts/python.exe -X utf8 numcog/explore_p0b.py     # VPN(super_class) + MBIN
.venv/Scripts/python.exe -X utf8 numcog/explore_p0c.py     # precise VPN->KC / ALPN->KC
.venv/Scripts/python.exe -X utf8 numcog/explore_p0d.py     # KCg-d parsing
```
Raw outputs: `numcog/results_p0/` (assembly + `vpn2kc_edges.csv` + `alpn2kc_edges.csv`).

**DECISION:** Waiting for transition to PHASE 1 (status — user confirmation).
