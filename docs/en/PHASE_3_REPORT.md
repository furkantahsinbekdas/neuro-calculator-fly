<!-- Machine translation of `numcog/RAPOR_FAZ_3.md` (local Ollama / gemma3:4b). The Turkish original is the authoritative record; this file is a reading copy.
     Numeric-token check: PASS (all 162 numeric tokens present) -->
...
- **H3.1 DESTEKLENDİ:** coarse_direct EĞİTİM = 0.778 < 1.0. Toplamsal (doğrusall) okuma
...
(combo=0.0). Yani tek op'la gördüğü n için (ör. n=4 op+→5) diğer op'u (n=4 op−→3) TAHMİN EDİYOR;
