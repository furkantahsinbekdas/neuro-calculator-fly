...
```bash
cd flyputer
.venv/Scripts/python.exe -X utf8 numcog/cx_spectral.py labels    # Adım 1
.venv/Scripts/python.exe -X utf8 numcog/cx_spectral.py embed     # Adım 2
.venv/Scripts/python.exe -X utf8 numcog/cx_spectral.py tests     # Adım 3 (1000 null, ~56 s)
.venv/Scripts/python.exe -X utf8 numcog/cx_spectral.py bonus     # bonus dairesel korelasyon
.venv/Scripts/python.exe -X utf8 numcog/cx_spectral.py merge     # Holm m=2 + karar sayıları
.venv/Scripts/python.exe -X utf8 numcog/cx_spectral.py explore   # KEŞİFSEL
```

Null örnekleri **deterministik tohum** kullanır (`RandomState(8000 + örnek_no)`) → **tek süreç**
(≤1 kuralı); derece korunumu her örnekte **assert** edilir.

Çıktılar (`numcog/results_p6_0b/`): `label_counts.csv`, `embedding_coords.csv`,
...
1. **Girdi:** EPG 51, PEN 42; tip sütunlarında NaN yok; **663 sinaps satırı → 582 benzersiz çift**,
   6.852 sinaps; EPG çıkışı olan 50/51. **Wedge/glomerül etiketi YOK** (sayılar yalnızca PEN alt tip
   adlarında: `PEN_a(PEN1)`, `PEN_b(PEN2)`).
2. **Graf bağlı değil:** 1 **izole EPG hücresi** (sol) + ana bileşen (50 EPG [25 sol + 25 sağ] +
   42 PEN); λ=0 **iki kez** (M ve S'de). Null'ların **%100'ü** de aynı yapıda → karşılaştırma eşleşmiş.
3. **Gömme (birincil):** M r_cv **1,1538**, g_max **86,2°**; S r_cv 1,6562, g_max 87,1°.
   (Literal "indis 1,2" sürümü: M 1,7668 / 92,3°; S 2,0597 / 93,7°.)
4. **H6b.1 (literal ölçüt) AYRIŞIR:** r_cv 1,1538 vs null 2,8476 ± 0,4753 (z = −3,56,
   p_holm = 0,0020). **Ama bu halka DEĞİLDİR:** r_cv ≈ 1,15 bulutsu/iki-loblu gömmedir (Faz 6-0
   soma tabanlı halkada r_cv = 0,336); ölçüt yalnızca "null'dan daha az ekstrem" demektedir →
   **halka iddiası desteklenmemiştir.**
5. **H6b.2 ÇÜRÜTÜLDÜ:** g_max **86,2° ≥ 60°** ve null'a göre aşırı değil (p_ham = 0,615) →
   **tam tur yok**.
6. **BONUS:** |r_c| = 0,300 (M) / 0,322 (S) → soma açısıyla **zayıf** uyum; işaret keyfî.
7. **KARAR:** **bu veri sürümünde, bu yöntemle** bağlantı matrisinden **halka koordinatı çıkmıyor**;
   wedge sayısı temiz değil (**eşiğe göre 6–18**); dinamik simülasyon **yapılmadı**.
8. **Zorunlu ifade (§7):** türetilen açılar **wedge etiketi değildir**; **sıfır noktası ve yön
   keyfîdir** (özvektör işareti/döndürmesi veriden gelmez).
