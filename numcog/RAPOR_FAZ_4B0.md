...
**Kritik:** klasik hız modelindeki **PEN→Delta7 bağlantısı yok denecek kadar az (5 kenar)**.
...
```
Çıktı: `numcog/results_p4b0/` (multiply.csv, multiply_error_steps.csv, cx_edges.csv).

**KARAR:** Halka simülasyonu mümkün değil (wedge etiketi yok; PEN→Δ7 yok; bağlantı yapısız).
Sonraki adım için bekleniyor (dur — kullanıcı onayı).
